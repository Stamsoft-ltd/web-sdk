...
class MacroJSX {
  constructor({ types }, opts) {
    ...
    __publicField$1(this, "isLinguiComponent", (path, name) => {
      ...
      return identifier.referencesImport(ast.MACRO_REACT_PACKAGE, name) || identifier.referencesImport(ast.MACRO_LEGACY_PACKAGE, name);
    });
 ...
 `
    ...
    path.scope.getBinding(_property.value.name)?.referencePaths.forEach((refPath) => {
      ...
      if (currentPath.isTaggedTemplateExpression()) {
        const tokens = ast.tokenizeTemplateLiteral(currentPath.node, ctx);
...
function linguiPlugin({
  ...
  function getMacroImports(path) {
    return path.get("body").filter((statement) => {
      return statement.isImportDeclaration() && [
        ast.MACRO_CORE_PACKAGE,
        ast.MACRO_REACT_PACKAGE,
        ast.MACRO_LEGACY_PACKAGE
  ...
  function isLinguiIdentifier(path, node, macro) {
    ...
    if (macro === ast.JsMacroName.useLingui) {
      ...
      ) || identPath.referencesImport(ast.MACRO_LEGACY_PACKAGE, ast.JsMacroName.useLingui)) {
        return true;
      }
    } else {
      identPath = identPath || getIdentifierPath(path.getFunctionParent(), node);
      if (identPath.referencesImport(ast.MACRO_CORE_PACKAGE, macro) || identPath.referencesImport(ast.MACRO_LEGACY_PACKAGE, macro)) {
        return true;
      }
    }
    return false;
