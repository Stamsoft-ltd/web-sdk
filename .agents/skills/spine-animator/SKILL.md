...
This skill includes Python scripts that do the heavy lifting. Codex MUST write them to disk
before use. Each script is embedded below — Codex should save them to `/home/Codex/spine-scripts/`
at the start of every session.

```bash
mkdir -p /home/Codex/spine-scripts
...
**Codex: read these carefully, then write each one to `/home/Codex/spine-scripts/`**
...
| **Full character image** | A single image — Codex will help define part regions |
...
```bash
GEMINI_API_KEY=your_key python /home/Codex/spine-scripts/split_character.py character.png \
  --output-dir parts/
```
...
1. **If separated PNGs**: Use Codex's vision to identify each part (head, torso, left-arm, etc.)
...
3. **If full image only**: Use Codex's vision to identify body parts, then crop into separate PNGs.
...
```bash
python3 /home/Codex/spine-scripts/position_parts.py \
...
Compare composite vs reference with Codex's vision and adjust layout JSON offsets.
...
```bash
python3 /home/Codex/spine-scripts/make_atlas.py \
  --parts parts_folder/ \
  --output . \
  --name character_name
```
...
```bash
python3 /home/Codex/spine-scripts/generate_spine_player.py \
...
This JSON can be fed back to Codex to apply corrections to the skeleton.