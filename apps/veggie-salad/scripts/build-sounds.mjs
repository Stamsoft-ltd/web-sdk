...
/**
 * Rebuilds the Howler audio sprite (static/assets/veggie-salad/audio/sounds.{mp3,ogg,json}) from individual
...
const outDir = join(appDir, 'static/assets/veggie-salad/audio');
...
// bgm_* tracks loop, as does anything named *_loop (played through the looping player) — Howler
// only loops a sprite whose [offset,duration,loop] flag is true.
const isLoop = (n) => n.startsWith('bgm_') || n.endsWith('_loop');
...
console.log(
	`sounds sprite rebuilt: ${provided}/${names.length} provided, ${missing.length} silent.`,
);
