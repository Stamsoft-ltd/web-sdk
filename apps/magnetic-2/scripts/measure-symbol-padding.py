...
THE WIDTH CAP
-------------
Equalising the limiting axis equalises HEIGHT, because every file has the cell's aspect and every
drawing is taller-relative-to-its-file than it is wide. A symbol drawn on an unusually WIDE
silhouette therefore ends up matching its class in height while overhanging it in width, and the eye
compares extents: the electromagnetic device measured 0.774 of the cell across against 0.70-0.72 for
the other three premiums, and read a size larger than all of them (user, 2026-09-03).

So after the limiting-axis pass, any symbol whose width exceeds its class MEDIAN width by more than
WIDTH_TOLERANCE is pulled back to that median. Only classes with at least three members are capped:
a median over two symbols is just the mean of a shape the designer chose to differ (the scatter
capsule and the wild horseshoe are 0.53 and 0.72 wide, and neither is wrong).

The cap makes the scale for such a symbol LESS than 1, which is the one case where this pass
shrinks. Every other scale is >= 1: nothing in the set overfills its canvas, and if a future symbol
is cut oversized the right fix is its build script, not a shrink here.
...
# How much wider than its class median a symbol may be before it is pulled back to it.
WIDTH_TOLERANCE = 0.06


def main():
    ratios = class_ratios()
    rows, audit, measured, capped = [], [], [], []
    ...
    for name, comp, stems in SETS:
        ...
        measured.append([name, cls, fw, fh, scale, keys])

    # Width cap -- see THE WIDTH CAP above. Widths are compared as a share of the CELL, so a class
    # ratio change moves the median with them.
    by_class = {}
    for name, cls, fw, fh, scale, _keys in measured:
        by_class.setdefault(cls, []).append(ratios[cls][0] * scale * fw)
    for row in measured:
        name, cls, fw, fh, scale, _keys = row
        widths = sorted(by_class[cls])
        if len(widths) < 3:
            continue
        mid = len(widths) // 2
        median = widths[mid] if len(widths) % 2 else (widths[mid - 1] + widths[mid]) / 2
        width = ratios[cls][0] * scale * fw
        if width > median * (1 + WIDTH_TOLERANCE):
            row[4] = scale * median / width
            capped.append((name, width, median))

    for name, cls, fw, fh, scale, keys in measured:
        scale = round(scale, 3)
        rw, rh = ratios[cls]
    ...
    print(f'// then caps any symbol more than {WIDTH_TOLERANCE:.0%} wider than its class median at that median.')
    ...
    for name, width, median in capped:
        print(f'-- width cap: {name} was {width:.3f} of the cell across,'
              f' pulled back to its class median {median:.3f}', file=sys.stderr)
