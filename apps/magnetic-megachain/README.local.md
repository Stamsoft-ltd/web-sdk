...
```bash
pnpm run dev --filter=magnetic-megachain
```
...
```bash
pnpm --dir apps/magnetic-megachain run storybook
```
...
```bash
pnpm run build-game -- magnetic-megachain
```

Expected output:
- `apps/magnetic-megachain/build/index.html`
...
## Magnetic Megachain placeholder media wired
...
- Branding card and loading art use Magnetic Megachain concept images.

## Known limit
- Premium/low symbol reels still use sample skeleton atlas placeholders because no per-symbol source sprites/spines were provided in `Magnetic Megachain_Project`.
...
- `apps/magnetic-megachain/static/forest-gang/extracted/`
