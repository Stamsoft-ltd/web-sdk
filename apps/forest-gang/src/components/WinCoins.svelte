<script lang="ts">
	...
	// Coin count ramps up with the win level: a tiny sprinkle for small / SWEET wins, climbing
	// steadily through the board tiers to a huge fountain at LEGENDARY. `max` = concurrent coins,
	// `freq` = seconds between spawns (lower = denser).
	//   small / nice / substantial → count-up only (no board)
	//   big=SWEET  superwin=WILD  mega=EPIC  epic=MYTHIC  max=LEGENDARY
