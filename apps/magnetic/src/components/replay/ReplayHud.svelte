<script lang="ts">
	...
	const replayNeedsStart = $derived(replayReady && !replayRunning && !magneticStakeState.replayStartRequested);
	const replayError = $derived(magneticStakeState.bootStatus === 'error' ? magneticStakeState.bootError : '');
	const replayBetText = $derived(magneticStakeDerived.formatCurrencyAmount(magneticStakeDerived.replayBetAmount()));
	const replayCostText = $derived(magneticStakeDerived.formatCurrencyAmount(magneticStakeDerived.replayCostAmount()));
	const replayPayoutText = $derived(
		magneticStakeDerived.formatCurrencyAmount(magneticStakeDerived.replayPayoutAmount()),
	);
	...
	const startReplay = () => {
		if (!magneticStakeDerived.requestReplayStart() || !magneticStakeState.replaySnapshot) return;
		stateBet.betToResume = structuredClone(magneticStakeState.replaySnapshot);
	...
	<div class="replay-hud">
		<div class="replay-topbar">
			<div class="replay-brand">
				<span class="replay-title">{magneticStakeDerived.t('GAME TITLE')}</span>
			...
			<div class="replay-meta">
				...
				{#if magneticStakeState.replayEventId}
					<span class="replay-chip">{magneticStakeDerived.t('EVENT')} {magneticStakeState.replayEventId}</span>
				{/if}
			</div>
		</div>
