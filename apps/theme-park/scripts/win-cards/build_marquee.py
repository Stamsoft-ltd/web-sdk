...
# Export widths. The plate is the one piece that fills the card, so it carries the most detail.
PLATE_W = 1400
TEXT_W = 1100
STAR_W = 220
CONFETTI_SCALE = 1.0  # confetti pieces keep their native resolution

# Bulbs painted into the plate art. Unlike the old panels this needs no region mask: the marquee's
# field is flat dark purple, so a brightness cut alone separates the bulbs from it. The one thing
# excluded is the tent's flag, whose gold knob is as bright as a bulb — anything above this fraction
# of the plate's height is flag, not frame.
FLAG_BELOW = 0.13
...
def peaks(img, percentile, min_gap, window=9):
    """Brightness peaks, at least `min_gap` (fraction of the long edge) apart. Lifted from
    build_bulbs.py — the marquee's bulbs are the same problem as the old panels'."""
...
def emit_ts(layout):
    lines = [
        ...
        f'export const MARQUEE_PLATE: PartRect = {rect_ts(layout["plate"])};',
        '',
...
def main():
    ...
    # === PLATE — the card itself, and the unit everything else is measured in ===
    plate_raw = load('plate_4.png')
    plate_box, plate_img = trim(plate_raw)
    ...
    layout['plate'] = place(plate_canvas, plate_raw.size, plate_box, card_w, card_cx, card_cy)
    save(resized(plate_img, PLATE_W), OUT / 'plate.webp')
    ...
    # === PLATE BULBS ===
    bulbs = [p for p in peaks(plate_img, 98.0, 0.016) if p[1] > FLAG_BELOW]
    pl = layout['plate']
    layout['plateBulbs'] = [
        [round(pl['x'] + (fx - 0.5) * pl['w'], 4), round(pl['y'] + (fy - 0.5) * pl['h'], 4)]
        for fx, fy in bulbs
    ]
    ...
    print(f'confetti pieces: {len(pieces)}   plate bulbs: {len(bulbs)}')
    for tier in TIERS:
        t = layout['text'][tier]
        print(f'  text {tier:10s} w={t["w"]:.3f} h={t["h"]:.3f}')

    # --- verify sheets ---
    bg = Image.new('RGBA', plate_img.size, (18, 5, 32, 255))
    bg.alpha_composite(plate_img)
    d = ImageDraw.Draw(bg)
    r = max(plate_img.size) * 0.009
    for fx, fy in bulbs:
        x, y = fx * plate_img.width, fy * plate_img.height
        d.ellipse([x - r, y - r, x + r, y + r], outline=(0, 255, 90, 255), width=3)
    bg.convert('RGB').save(BASE / 'verify_marquee_bulbs.png')
