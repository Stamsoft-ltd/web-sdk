<script lang="ts">
	...
	type Props = {
		/** 'confirm' = 467x225 two-button dialog; 'wide' = 633x464 settings panel. */
...
<div class="tp-popup-overlay" data-variant={variant}>
	...
	<div class="tp-popup" data-variant={variant} role="dialog" aria-modal="true">
		<!-- Two flat plates and a hairline, drawn rather than painted — neither popup has a glow to
		     carry or lights to run any more, so there is nothing left for an export to hold. -->
...
<style>
	...
	.tp-popup[data-variant='wide'] {
		--pop-w: 633;
		width: clamp(320px, 58vw, 633px);
	}

	/* The redesign's popup plate, Figma 7063:18550: a #63307d plate with a #21003D one inset 4 inside
	   it and a #5E4374 hairline around that. It replaces a neon-edged export — a painted glow with
	   running bulbs down its border — which the confirm dialog shed first and the wide panel wore
	   until the autoplay screen was brought in line with it. Drawn rather than exported because
	   there is nothing here a raster holds better than two rounded rectangles, and the radii then
	   follow the panel's clamp instead of stretching with it. Both variants state their radius in
	   design units, so the 14 is 14 design px in a 467 dialog and in a 685 panel alike. */
	.tp-popup__plate {
		position: absolute;
		inset: 0;
	...
	/* Wide, Figma 7063:18400: 633x464 with the same 494-wide content column the 685x537 panel had —
	   the redesign kept every control at its old size and shrank the plate around it, so the column
	   is now 69.5 in from each side rather than 95.5. First row 42 down, 36 left below. */
	.tp-popup[data-variant='wide'] .tp-popup__content {
		min-height: calc(464 / var(--pop-w) * 100cqw);
		padding: calc(42 / var(--pop-w) * 100cqw) calc(69.5 / var(--pop-w) * 100cqw)
			calc(36 / var(--pop-w) * 100cqw);
	}
	...
	/* Lilita One 32 / 0.96 tracking, white (node 7063:18552). It used to be the magenta-to-blue
	   ...
	   the eye to land on and leaves the heading plain. Every popup names itself with this now — the
	   wide panel took a heading when it lost the painted frame that used to fill its top band, the
	   same way the redesigned bonus screen titles itself. Row labels are .tp-popup__label. */
	...
	/* 50 tall, radius 12, 24 of side padding, Lilita One 16 / 1.4 tracking — the size both panels
	   now share (nodes 7063:18554-18557 for the dialog, 7063:18415 for the wide panel's action), in
	   the redesign's fill: the inner plate's #21013d edged in the outer plate's
	   #63307d, so a secondary button reads as part of the panel rather than as a second dark chip
	   with a lilac hairline. Nothing on these panels casts a shadow now, so neither does this. */
	:global(.tp-popup__btn) {
		...
		/* 1.4px at 16 — stated in design units so it tracks the panel, not the font. */
	...
	/* The one control the eye should land on keeps the gradient, edged in the gradient's own magenta. */
