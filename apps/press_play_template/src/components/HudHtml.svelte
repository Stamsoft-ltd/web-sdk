<script lang="ts">
	...
	const formattedBalance = $derived(templateStakeDerived.formatCurrencyAmount(stateBet.balanceAmount));
...
<div
	...
	style={`--menu-btn-bg:url('${menuBtnFrame}');--sound-btn-bg:url('${soundBtnFrame}');--menu-bar-bg:url('${menuBarFrame}');--hud-frame-bg:url('${hudFrame}');--buy-btn-bg:url('${buyBtnFrame}');--small-btn-bg:url('${smallBtnFrame}');--play-btn-bg:url('${playBtnFrame}')`}
>
...
{#if showBuyModal}
	<CustomBuyBonusModal onclose={() => (showBuyModal = false)} />
{/if}
